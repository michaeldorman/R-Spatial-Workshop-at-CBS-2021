...
In our case, we are only interested in the subset of population estimates per age category:
...
One thing we may notice in Figures \@ref(fig:stat-layer) and \@ref(fig:stat-one-attr), is that many of the statistical areas have `NA` values, meant to represent zero (rather than unknown) population size. For all practical purposes, these `NA` values should be replaced with `0` values:
...
Figure \@ref(fig:stat-one-attr) shows one of the attributes in `stat` with the `nafot` layer on top (in black), demonstrating that the layers are aligned:
