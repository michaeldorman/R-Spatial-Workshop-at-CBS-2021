...
## `RPostgreSQL`: Working with PostGIS {#rpostgresql-working-with-postgis}
...
Commonly used vector layer file formats (Table \@ref(tab:vector-formats)) include binary formats (such as the **Shapefile**) and plain text formats (such as **GeoJSON**). Vector layers are also frequently kept in a spatial database, such as **PostgreSQL/PostGIS**. 
...
## `sf` data structures {#the-sf-package}
...
```{r simple-plot-geometry, fig.cap="The `nafot` layer", fig.width=4, fig.height=6, out.width="50%"}
...
* Transform several layers into the same projection, so that they can be displayed one on top of the other (e.g., Figure \@ref(fig:sfg-plot-add)) or so that they can be subject to a spatial operator (e.g., Figure \@ref(fig:intersection-geometrycollection))
