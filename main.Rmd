...
## [`sf`](https://cran.r-project.org/package=sf) and [`stars`](https://cran.r-project.org/package=stars) {#input-and-output-of-spatial-data}
...
## [`sf`](https://cran.r-project.org/package=sf): Vector Layers
...
## [`stars`](https://cran.r-project.org/package=stars): Rasters
...
## [`gstat`](https://cran.r-project.org/package=gstat): Interpolation
...
## [`spdep`](https://cran.r-project.org/package=spdep): Spatial dependence
...
## [`spatstat`](https://cran.r-project.org/package=spatstat): Point patterns
...
## [`RPostgreSQL`](https://cran.r-project.org/package=RPostgreSQL): PostGIS {#rpostgresql-working-with-postgis}
...
Package `sf` (Section \@ref(the-sf-package)) combined with `RPostgreSQL` can be used to read from, and write to, a **PostGIS** spatial database. First, we need to create a connection object:
...
```{r sf-dependencies, echo=FALSE, fig.cap="`sf` package dependencies (source: https://github.com/edzer/rstudio_conf)", out.width="100%"}
...
```{r nc-geometry-column, echo=FALSE, fig.cap="Structure of an `sf` object (source: https://cran.r-project.org/web/packages/sf/vignettes/sf1.html)", out.width="100%"}
...
The result is a new line layer of `r nrow(rail)` `MULTILINESTRING` geometries, including the `name_eng` attribute of the corresponding "Nafa" they intersect with:
...
To transfer the track lengths we just calculated to the polygonal `nafot` layer, we use a non-spatial *join*. First, we extract the attribute table of `rail`, using `st_drop_geometry` (Section \@ref(reading-vector-data)):
...
Plotting the layer shows the new `density` attribute (Figures \@ref(fig:density-1) on its own: 

```{r density-1, fig.cap='Railway density per "Nafa"', out.width="50%", warning=FALSE, fig.width=4, fig.height=6}
plot(nafot["density"])
```

or along with \@ref(fig:density-2)) the other attributes:
...
The resulting subset of the `stat` layer, now with just `r length(vars)` attributes, is shown in Figure \@ref(fig:stat-layer).
...
One thing we may notice in Figure \@ref(fig:stat-layer) (or printing `stat` in the console) is that many of the statistical areas have `NA` values, meant to represent zero (rather than unknown) population size (e.g., in open areas). For all practical purposes, these `NA` values should be replaced with `0` values:

```{r}
stat[is.na(stat)] = 0
```

The modified `stat` layer is shown in Figure \@ref(fig:stat-layer-filled):
...
Next, we are going to calculate the average age structure per "Nafa" (`nafot`), based on the data from statistical areas (`stat`). First of all, we need to make sure both layers are in the same CRS (Section \@ref(vector-layer-reprojection)):

```{r}
stat = st_transform(stat, st_crs(nafot))
```

Figure \@ref(fig:stat-one-attr) shows one of the attributes in `stat` (colored polygons with narrow borders) with the `nafot` layer on top (transparent polygons with thick black bordwer), demonstrating that the layers are aligned:
...
* `extensive`---Does the layer contains [spatially extensive](https://en.wikipedia.org/wiki/Choropleth_map#Property:_aggregate_statistical_summaries) variables, which are summed, such as population (`TRUE`), or spatially intensive variables which are averaged, such as population density (`FALSE`)?
...
The resulting layer `x` has the `nafot` geometries, along with summed population estimates from `stat`. It is shown in Figure \@ref(fig:aw-nafa):
...
```{r hc-dendrogram, fig.cap="Hierarchical dendrogram of age structure per Nafa", fig.width=7, fig.height=5, out.width="100%"}
plot(hc)
rect.hclust(hc, k = k)
```
...
```{r hc-map, fig.cap="Age structure clusters on a map", fig.width=4, fig.height=6, out.width="50%"}
...
and then use [`ggplot2`](https://cran.r-project.org/package=ggplot2) (Figure \@ref(fig:ggplot-1)).

```{r ggplot-1, fig.cap="Distribution of age structures in each cluster of `nafot`", fig.width=12, fig.height=4, out.width="100%"}
library(ggplot2)
library(ggrepel)
